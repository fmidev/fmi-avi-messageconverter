...
public class ConversionChainBuilder<S> {
    ...
    private ConversionResult.Status requiredStatus;

    public ConversionChainBuilder(final AviMessageConverter converter, final ConversionSpecification<S,?> initialStep, final ConversionResult.Status
            requiredStatus) {
    ...
    public <U, T> ConversionChainBuilder<S> withMutator(final Function<U, T> mutator, final Class<U> inputClz, Class<T> outputClz) {
        this.converters.add(new AviMessageSpecificConverter<U, T>() {
            @Override
    ...
    public <T> AviMessageSpecificConverter<S, T> build(final ConversionSpecification<?, T> finalStep) {
        ...
        return (input, hints) -> {
            ...
            for (int i = 1; i < converters.size(); i++) {
                ...
                Object msg = result.getConvertedMessage().get();
    ...
    private ConversionResult<?> convertSingleMessage(final Object msg, final ConversionSpecification spec, final AviMessageSpecificConverter converter, final
            ConversionHints hints) {
        ConversionResult<?> retval = new ConversionResult<>();
        if (!spec.getInputClass().isAssignableFrom(msg.getClass())) {
            retval.addIssue(new ConversionIssue(ConversionIssue.Severity.ERROR, ConversionIssue.Type.OTHER, "Cannot use intermediate result of "
                    + "type " + msg.getClass().getCanonicalName() + " as input for the conversion step " + spec));
